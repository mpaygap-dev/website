...
import { callAll, noop } from '@/lib/fn-lib';
import { formatMoney } from '@/lib/number';
import { isDefined, isNumber } from '@/lib/type-guard';

import { TextInput, TextInputProps } from './text-input';

export interface NumberInputProps
  extends Omit<TextInputProps, 'value' | 'onChangeValue' | 'type' | 'max'> {
  value: string;
  /**
...
const NumberInputImpl = function NumberInput({
  className,
  decimalPlaces = 2,
  onChangeValue = noop,
  allowTrailingZero,
  ...
  const { value, onChange } = useRifm({
    ...
    onChange: (newVal) =>
      onChangeValue(
        parseNumber(newVal, decimalPlaces === 0, max, allowNegative)
      ),
    accept: /[\d.-]/g,
    format: (val) =>
